...
public class LayeredArchitectureTest {
    ...
    private static final String LISTENER_LAYER = "Listener";
    ...
    private static final String LISTENER_PACKAGE = ROOT_PACKAGE + "..listener..";
    ...
    private static final String EVENT_PACKAGE = ROOT_PACKAGE + "..event..";
    ...
    private static final String DESC_LAYERED_ARCHITECTURE = "Controller, Listener, Service, Repository, Entity, Event 간의 계층형 아키텍처 제약(EDA 포함)을 준수해야 한다";
    ...
    void testLayeredArchitecture() {
        Architectures.LayeredArchitecture architecture = layeredArchitecture()
                ...
                .layer(LISTENER_LAYER).definedBy(LISTENER_PACKAGE)
                ...
                // Controller와 Listener는 진입점이므로 내부 계층에서 접근할 수 없음
                .whereLayer(CONTROLLER_LAYER).mayOnlyBeAccessedByLayers()
                .whereLayer(LISTENER_LAYER).mayOnlyBeAccessedByLayers()

                // Service는 진입점(Controller, Listener) 및 시뮬레이터에서만 접근 가능
                .whereLayer(SERVICE_LAYER).mayOnlyBeAccessedByLayers(CONTROLLER_LAYER, LISTENER_LAYER, SIMULATION_LAYER)
                ...
                .whereLayer(ENTITY_LAYER).mayOnlyBeAccessedByLayers(REPOSITORY_LAYER, SERVICE_LAYER, CONTROLLER_LAYER, LISTENER_LAYER, INFRASTRUCTURE_LAYER, SIMULATION_LAYER)

                // Event는 모듈 간 통신을 위해 발행(Service) 및 구독(Listener) 계층에서 모두 접근 가능
                .whereLayer(EVENT_LAYER).mayOnlyBeAccessedByLayers(SERVICE_LAYER, LISTENER_LAYER, CONTROLLER_LAYER, SIMULATION_LAYER);
