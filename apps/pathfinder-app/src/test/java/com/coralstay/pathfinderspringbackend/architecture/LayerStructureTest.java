...
public class LayerStructureTest {
    ...
    private static final String EVENT_PACKAGE = "..event..";
    ...
    private static final String DESC_EVENT_SHOULD_NOT_DEPEND_ON_OTHER_LAYERS = "event 패키지 클래스는 순수 데이터/메시지이므로 다른 레이어 로직(Service, Controller 등)에 의존하면 안 된다";
    private static final String DESC_LISTENER_SHOULD_NOT_DEPEND_ON_INVALID_LAYERS = "listener 패키지는 Service를 통해서만 로직을 수행해야 하며 Controller, Repository, Infrastructure에 직접 의존하면 안 된다";

    private static final String DESC_INTRA_MODULE_LAYER_TEST = "Controller → Service → Repository → Domain 의존성 제약";
    private static final String DESC_CONTROLLER_SHOULD_ONLY_ACCESS_SERVICE = "Controller는 Service에만 의존해야 한다 (Repository, Entity, Listener 직접 접근 금지)";
    private static final String DESC_SERVICE_SHOULD_NOT_DEPEND_ON_CONTROLLER_OR_LISTENER = "Service는 Controller나 Listener(진입점)에 의존하면 안 된다";
    private static final String DESC_REPOSITORY_SHOULD_NOT_DEPEND_ON_UPPER_LAYERS = "Repository는 Controller, Service, Listener에 의존하면 안 된다";
    ...
    class LayerIsolationTest {
        ...
        void entityShouldNotDependOnOtherLayers() {
            noClasses()
                    ...
                    .should().dependOnClassesThat().resideInAnyPackage(
                            INFRASTRUCTURE_PACKAGE, SIMULATION_PACKAGE, REPOSITORY_PACKAGE, DTO_PACKAGE, CONTROLLER_PACKAGE, SERVICE_PACKAGE, LISTENER_PACKAGE)
        ...
        void dtoShouldNotDependOnOtherLayers() {
            noClasses()
                    ...
                    .should().dependOnClassesThat().resideInAnyPackage(
                            INFRASTRUCTURE_PACKAGE, SIMULATION_PACKAGE, REPOSITORY_PACKAGE, ENTITY_PACKAGE, CONTROLLER_PACKAGE, SERVICE_PACKAGE, LISTENER_PACKAGE)
                    .allowEmptyShould(true)
                    .check(classes);
        }
        ...
        void repositoryShouldOnlyDependOnEntity() {
            noClasses()
                    ...
                    .should().dependOnClassesThat().resideInAnyPackage(
                            INFRASTRUCTURE_PACKAGE, SIMULATION_PACKAGE, DTO_PACKAGE, CONTROLLER_PACKAGE, LISTENER_PACKAGE)
                    .allowEmptyShould(true)
                    .check(classes);
        }
        ...
        void eventShouldNotDependOnOtherLayers() {
            noClasses()
                    ...
                    .should().dependOnClassesThat().resideInAnyPackage(
                            CONTROLLER_PACKAGE, SERVICE_PACKAGE, REPOSITORY_PACKAGE, INFRASTRUCTURE_PACKAGE, SIMULATION_PACKAGE, LISTENER_PACKAGE)
                    .allowEmptyShould(true)
                    .check(classes);
        }

        @Test
        @DisplayName(DESC_LISTENER_SHOULD_NOT_DEPEND_ON_INVALID_LAYERS)
        void listenerShouldNotDependOnInvalidLayers() {
            noClasses()
                    .that().resideInAPackage(LISTENER_PACKAGE)
                    .should().dependOnClassesThat().resideInAnyPackage(
    ...
    class IntraModuleLayerTest {
        ...
        void controllerShouldOnlyAccessService() {
            noClasses()
                    ...
                    .should().dependOnClassesThat().resideInAnyPackage(
                            REPOSITORY_PACKAGE, ENTITY_PACKAGE, INFRASTRUCTURE_PACKAGE, LISTENER_PACKAGE)
                    .allowEmptyShould(true)
                    .check(classes);
        }

        @Test
        @DisplayName(DESC_SERVICE_SHOULD_NOT_DEPEND_ON_CONTROLLER_OR_LISTENER)
        void serviceShouldNotDependOnControllerOrListener() {
            noClasses()
                    .that().resideInAPackage(SERVICE_PACKAGE)
                    .should().dependOnClassesThat().resideInAnyPackage(CONTROLLER_PACKAGE, LISTENER_PACKAGE)
                    .allowEmptyShould(true)
                    .check(classes);
        }
        ...
        void repositoryShouldNotDependOnUpperLayers() {
            noClasses()
                    .that().resideInAPackage(REPOSITORY_PACKAGE)
                    .should().dependOnClassesThat().resideInAnyPackage(
                            CONTROLLER_PACKAGE, SERVICE_PACKAGE, LISTENER_PACKAGE)
                    .allowEmptyShould(true)
                    .check(classes);
        }
    }
